...
export async function GET() {
  const res = await db.query(`SELECT * FROM discounts ORDER BY created_at DESC`);
...
export async function POST(req: Request) {
  ...
  const { name, type, value, start_date, end_date, active, product_ids } = await req.json();
  ...
  try {
    ...
    return new NextResponse(err.message || "Error creating discount", { status: 500 });
  } finally {
    client.release();
  }
}
